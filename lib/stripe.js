...
export async function getUserPlan(userId) {
  ...
  const rows = isPostgres
    ? await prisma.$queryRawUnsafe(`SELECT email, plan, plan_status, plan_ends_at FROM "User" WHERE id = $1`, parseInt(userId))
    : await prisma.$queryRawUnsafe(`SELECT email, plan, plan_status, plan_ends_at FROM "User" WHERE id = ?`, parseInt(userId))
  const user = rows[0]
  if (!user) return 'free'
  if (isAdminEmail(user.email)) return 'lifetime'   // Admin → immer voller Zugriff
